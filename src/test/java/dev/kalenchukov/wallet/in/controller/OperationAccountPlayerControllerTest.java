...
@WebMvcTest(OperationAccountPlayerController.class)
public class OperationAccountPlayerControllerTest {
	...
	public void beforeEach() {
		this.mockMvc = MockMvcBuilders.standaloneSetup(
						new OperationAccountPlayerController(this.operationService, this.authToken))
	...
	public class Get {
		...
		public void getValid() throws Exception {
			...
			mockMvc.perform(
							get("/players/{playerId}/accounts/{accountId}/operations/{operationId}", playerId, accountId,
									operationId
							).header("Authorization", "srd7b6s65Vr65E6WX4W65RV7878n897T7R65rsd7f6"))
					.andExpect(status().isOk())
					.andExpect(content().contentType(MediaType.APPLICATION_JSON))
					.andExpectAll(jsonPath("$.operationId").value(operationId), jsonPath("$.playerId").value(playerId),
		...
		public void getWithNotFound() throws Exception {
			...
			long playerId = 88L;
			long accountId = 14L;
			when(operationService.findById(anyLong(), anyLong(), anyLong())).thenThrow(
					new NotFoundOperationException(operationId));
			when(authToken.verifyToken(anyString())).thenReturn(playerId);
		...
		@Test
		public void getWithNoAccess() throws Exception {
			long operationId = 1L;
			long playerId = 88L;
			long accountId = 14L;
		...
		@Test
		public void getWithNoAccessToken() throws Exception {
			long operationId = 1L;
			long playerId = 88L;
			long accountId = 14L;
		...
		@Test
		public void getWithEmptyAccessToken() throws Exception {
			long operationId = 1L;
			long playerId = 88L;
			long accountId = 14L;
			...
			mockMvc.perform(
					get("/players/{playerId}/accounts/{accountId}/operations/{operationId}", playerId, accountId,
							operationId
					).header("Authorization", "")).andExpect(status().isForbidden()).andExpectAll(
					jsonPath("$.message").isNotEmpty());
		}

		@DisplayName("Проверка с некорректным токеном доступа.")
		@Test
		public void getWithInvalidAccessToken() throws Exception {
			long operationId = 1L;
			long playerId = 88L;
			long accountId = 14L;
			...
			mockMvc.perform(
					get("/players/{playerId}/accounts/{accountId}/operations/{operationId}", playerId, accountId,
							operationId
					).header("Authorization", "7s5fvb65df7bs7dft8")).andExpect(status().isForbidden()).andExpectAll(
					jsonPath("$.message").isNotEmpty());
		}
	}
	...
	public class Find {
		...
		public void findValid() throws Exception {
			...
			mockMvc.perform(
					get("/players/{playerId}/accounts/{accountId}/operations", playerId, accountId).header("Authorization",
					...
					jsonPath("$.length()").value(1), jsonPath("$[0].operationId").value(operationId),
					jsonPath("$[0].playerId").value(playerId), jsonPath("$[0].accountId").value(accountId),
		...
		public void findWithNoAccessToken() throws Exception {
			...
			mockMvc.perform(get("/players/{playerId}/accounts/{accountId}/operations", playerId, accountId)).andExpect(
					status().isBadRequest());
		}
